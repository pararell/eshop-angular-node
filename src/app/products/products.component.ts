...
export class ProductsComponent {
  ...
  addToCart(id) {
    ...
    .subscribe(lang => {
      this.store.dispatch(new actions.AddToCart(id + '/' + lang));
    });
  }
  ...
  private _loadProducts(): void {
    ...
    .subscribe(({lang, category, page, sort}) => {
      if (category) {
        this.store.dispatch(new actions.LoadCategoryProducts({lang, category, page: page || 1, sort: sort || 'newest' }));
      } else {
        this.store.dispatch(new actions.LoadProducts({lang, page: page || 1, sort: sort || 'newest' }));
      }
    });
  }
